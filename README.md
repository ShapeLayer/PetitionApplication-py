...
자동 생성된 다량 청원 방지 대책으로 reCaptcha v2가 추가되어있으며, 익명 청원에 대한 법적 문제 해결을 위해 익명 사용자의 명의를 기록, 확인하는 기능도 추가되어 있습니다. reCaptcha v2 API 키 정보를 `oauthsettings.json`에 추가하지 않으면 청원 작성 기능이 제대로 작동하지 않을 수 있습니다. [자세한 내용 확인하기](#api-%ED%82%A4-%EC%84%A4%EC%A0%95) **1.2.2 이후 변경될 예정입니다.** 이제 reCaptcha v2 정보를 입력하지 않으면 reCapthcha v2를 비활성화 한 것으로 처리됩니다.
